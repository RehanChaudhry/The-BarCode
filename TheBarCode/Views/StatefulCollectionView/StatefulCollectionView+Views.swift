//
//  StatefulCollectionView+Views.swift
//  Pods
//
...
extension StatefulCollectionView {
  ...
  internal var viewForInitialLoad: UIView? {
    if let delegateMethod = statefulDelegate?.statefulCollectionViewViewForInitialLoad {
  ...
  internal func viewForEmptyInitialLoad(withError error: NSError?) -> UIView? {
    if let delegateMethod = statefulDelegate?.statefulCollectionViewInitialErrorView {
